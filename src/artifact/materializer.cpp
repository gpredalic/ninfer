...
MaterializedArtifact materialize(const Reader& reader, const MaterializationPlan& plan,
    ...
    for (int index = 0; index < device_count; ++index) {
        ...
        if (device_count > 1) {
            ...
            // them. cudaMemsetAsync on transfer_stream orders the fill strictly before every copy.
    ...
    for (int index = 0; index < device_count; ++index) {
        CUDA_CHECK(cudaStreamSynchronize(devices[static_cast<std::size_t>(index)]->transfer_stream));
    }
