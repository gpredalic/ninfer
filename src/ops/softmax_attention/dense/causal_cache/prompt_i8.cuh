...
// PackGQA variant: one CTA per KV head. Q rows are packed as (token * G + qh_in_group),
// so a Br=64-row tile spans multiple Q heads within one KV head group and the K/V tile
// is loaded once for all G Q heads. The SMEM layout, warp schedule, quantization, and
// MMA structure are identical to the per-Q-head kernel above; only the Q load and the
// output scatter decode (q_head, token) from the packed row position.
template <typename Geometry, typename Metadata>
__global__ __maxnreg__(120) void causal_attention_prompt_i8_gqa_kernel(
    const __nv_bfloat16* __restrict__ q, const std::int8_t* __restrict__ cache_k,
    const std::int8_t* __restrict__ cache_v, const __half* __restrict__ cache_k_scale,
    const __half* __restrict__ cache_v_scale, Metadata metadata,
    const std::int32_t* __restrict__ positions, float scale, __nv_bfloat16* __restrict__ out,
    ...
    // Early exit: every packed row in this tile is past the last valid token. Zero the
    // G Q heads at these token positions, then return.
    if (packed_q0 >= G * tokens) {
        const int row_end = min(Br, G * width - packed_q0);
        for (int row = tid; row < row_end; row += kCausalPromptI8Threads) {
            ...
            if (token_r < width) {
                for (int d = lane; d < D; d += 32) {
                    out[causal_prompt_q_index<Geometry>(q_head, d, token_r)] =
    ...
    // Packed rows are token-major: packed = token * G + qh_in_group, so token = packed / G.
    // Rows below tile_rows are valid; rows at or above carry zero Q (padding).
    const int tile_rows     = max(0, min(Br, G * tokens - packed_q0));
    const int min_token     = packed_q0 / G;
    const int max_query_abs = base_pos + (packed_q0 + tile_rows - 1) / G;
    const int key_blocks    = max_query_abs / Bc + 1;
    ...
    // Quantize Q cooperatively. One full warp rotates and encodes one D256 row at a time;
    // each row's (q_head, token) is decoded from its packed position.
    for (int row = warp; row < Br; row += kCausalPromptI8Warps) {
        const int packed_r = packed_q0 + row;
        const int token_r  = packed_r / G;
        const int qh_r     = packed_r % G;
...
#pragma unroll
        for (int r = 0; r < 8; ++r) {
            ...
            if (row < tile_rows) {
                q_values[r] =
                    __bfloat162float(q[causal_prompt_q_index<Geometry>(q_head_r, d, token_r)]);
            }
        }
        normalized_hadamard_d256_inplace(q_values, lane);
...
#pragma unroll
            ...
            // Per-row causal mask: row r's query position is base_pos + token,
            // where token = (packed_q0 + r) / G (packed rows are token-major).
            const int row0             = row_base + gid;
            const int row1             = row0 + 8;
...
#pragma unroll
    for (int n = 0; n < PVNtPerWarp; ++n) {
        const int d0 = (d_slice * PVNtPerWarp + n) * 8 + 2 * lid;
        // Row 0: scatter to (q_head, token) derived from the packed position.
        {
            ...
            if (row0 < tile_rows) {
                *reinterpret_cast<unsigned*>(
                    &out[causal_prompt_q_index<Geometry>(q_head, d0, token_r)]) =
                    pack_bf16x2(acc[n][0] * inv_l0, acc[n][1] * inv_l0);
            } else if (token_r < width) {
                *reinterpret_cast<unsigned*>(
                    &out[causal_prompt_q_index<Geometry>(q_head, d0, token_r)]) = 0u;
            }
        }
        // Row 1.
        {
