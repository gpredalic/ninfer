...
void causal_attention_prompt_attention_launch_for(const Tensor& q, const Tensor& positions,
    ...
    static const cudaError_t attr_i8_gqa =
        cudaFuncSetAttribute(causal_attention_prompt_i8_gqa_kernel<Geometry, Metadata>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, kCausalPromptI8SmemBytes);
    ...
    if (cache.dtype == DType::I8) {
        ...
        if constexpr (Geometry::GroupSize > 1) {
            // PackGQA: one CTA per KV head. The Br=64 rows pack (token, qh-in-group) pairs, so
            // every K/V tile is loaded once and shared by all G Q heads of the group.
            const dim3 attention_grid(
                static_cast<unsigned>(div_up(tokens * Geometry::GroupSize, kCausalPromptI8Br)),
                static_cast<unsigned>(Geometry::KVHeads), 1u);
            causal_attention_prompt_i8_gqa_kernel<Geometry, Metadata>
