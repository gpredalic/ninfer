...
template <class Geometry>
void launch_tma(const Nvfp4W4a4Workspace& scratch, const Weight& weight, Tensor& out,
    ...
    if constexpr (std::is_same_v<Geometry, Nvfp4MlpGateUpGeometry>) {
        ...
        launch_nvfp4_linear_swiglu_w4a4_tma_shard(
            scratch.codes, scratch.scales, static_cast<const std::uint8_t*>(weight.qdata),
            static_cast<const std::uint8_t*>(weight.scales),
            static_cast<__nv_bfloat16*>(out.data), tokens, alpha, stream);
    }
...
void dispatch_impl(const Tensor& x, const Weight& weight, Tensor& out, LinearPolicy policy,
    ...
    case Nvfp4LinearSwiGluRoute::TmaFusedW4A4: {
        ...
        launch_tma<Geometry>(scratch, weight, out, x.ne[1], alpha, stream);
        return;
    }
