...
// stem hands ops::linear_row_parallel the two unpacked halves directly. It is therefore not
// allocated at tp == 2 -- a dead allocation is only wasted workspace, but the capacity query is
// derived from this same recipe, so leaving it in would also inflate the reported per-device
